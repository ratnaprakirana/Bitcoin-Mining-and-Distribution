        defmodule Bitcoin.Project4 do
            ...
            def process() do
                ...
                server_id=spawn_link(fn() -> coin_print(3,count,pid,total) end)
                ...
                for _ <- 1..logproc do
                    Task.Supervisor.async(pid,fn() -> miner(server_id,3) end)
                end
                receive do: (_ -> :ok)
            end
            ...
            def coin_print(k,count,pid,total) do
                receive do
                    msg ->
                    IO.puts "#{count} #{msg}"
                end
            ...
            # checking if the number of coins mined reached the required total of the bitcoins requested
                if count<=total do
                    coin_print(k,count+1,pid,total)
                else
                    wallet_block(count,pid)
                end
            end
            ...
            def wallet_block(count,pid) do
                ...
                k=0
                participant = Enum.random(4..10)
                IO.puts ("Total participants = #{participant}")
                total = count
                list = wallet_chain(count ,list,k, total,participant)
                #IO.inspect(list)
                transaction(list,participant)
                Process.exit(pid,:kill)
            end

            # Creates wallet for each participant with their own private and public key
            def wallet_chain(count,list,k , total , participant) do
                ...
                if k==participant-1 do
                    ...
                    len = trunc(:math.floor(total/(participant-1)))
                    len1 = trunc(:math.floor(total/(participant+1)))
                    ...
                    wallet_chain(count - a,list,k+1,total,participant)
                end
            end
            ...
            def transaction(list , participant) do
                t=Enum.random(4..10)
                ...
                q=1
                transactionlist(list , participant ,t ,q,invoice)
            ...
            def transactionlist(list , participant ,t , q , invoice) do
                    ...
                    if(a==b)do
                        ...
                        if val do
                            ...
                            if (amount <= amount1) do
                                ...
                                transactionlist(list,participant,t-1 , q+1 ,invoice )
                            else
